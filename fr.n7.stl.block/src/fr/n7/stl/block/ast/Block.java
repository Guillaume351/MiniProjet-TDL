 ...
 * language. Declares the various semantics attributes for the node.
 *
 * A block contains declarations. It is thus a Scope even if a separate
 * SymbolTable is used in the attributed semantics in order to manage
 * declarations.
 *
 * @author Marc Pantel
 *
...
public class Block {
	...
	public boolean collect(HierarchicalScope<Declaration> _scope) {
		...
		for (Instruction i : this.instructions) {
			...
			if (!ok) {
				Logger.error("Instruction " + i + " failed to collect");
			}
		}
	 ...
	 *         in the block have been previously defined.
	 */
	public boolean resolve(HierarchicalScope<Declaration> _scope) {
		...
		for (Instruction i : this.instructions) {
			...
			if (!ok) {
				Logger.error("Instruction " + i + " failed to resolve");
			}
		}
